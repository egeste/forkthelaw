...
def discover(state: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    logger.warning("Congress.gov discovery not implemented")
    return []
...
def run_pipeline(db, limit: Optional[int] = None) -> Dict[str, Any]:
    logger.info("Congress.gov pipeline (not yet implemented)")
    return {'source': 'congress_gov', 'status': 'not_implemented'}
